...
export default function StatsForm({ initialData }: { initialData: any[] }) {
    const [isSaving, setIsSaving] = useState(false);
    const [items, setItems] = useState<any[]>(
        Array.isArray(initialData) && initialData.length > 0
            ? initialData
            : [
                { id: "1", number: "100K+", label: "Case Closed" },
                { id: "2", number: "99.9%", label: "Success Rate" },
                { id: "3", number: "30+", label: "Years Experience" }
            ]
    );

    const handleAddItem = () => {
        setItems([...items, { id: crypto.randomUUID(), number: "", label: "" }]);
    };
    ...
    const handleSave = async () => {
        setIsSaving(true);
        const { error } = await updateSiteContent("stats", items);
        if (error) {
            toast.error("Failed to save stats: " + error);
        } else {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2">Fun Facts & Statistics</h2>
                ...
                {items.map((item, idx) => (
                    <div key={idx} className="p-4 border border-gray-200 dark:border-gray-700 rounded-xl space-y-4 relative bg-gray-50 dark:bg-gray-800/50 flex gap-4 items-center">
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Statistic Number</label>
                                <input
                                    type="text"
                                    value={item.number}
                                    onChange={(e) => handleUpdateItem(idx, "number", e.target.value)}
                                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 font-bold text-lg"
                                    placeholder="e.g. 100K+"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Statistic Label</label>
                                <input
                                    type="text"
                                    value={item.label}
